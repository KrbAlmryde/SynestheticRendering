...
uniform sampler1D	lut;			//volume dataset
...
void main()
{
    ...
    //Initialize vFragColor to 0 to avoid any weird artifacts
    vFragColor = vec4(0.0);
    ...
    for (int i = 0; i < MAX_SAMPLES; i++) {
        ...
        if( (sample -isoValue) < 0  && (sample2-isoValue) >= 0.0)  {
            ...
            vec3 tfColor = texture(lut, sample).rgb;
            ...
            //iso-surface
            vFragColor =  PhongLighting(L,N,V,250, tfColor);
            break;
        } 
    } 
}